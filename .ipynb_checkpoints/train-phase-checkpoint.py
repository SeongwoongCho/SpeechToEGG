...
##saving path
save_path = './models/sep/phase-only/{}/'.format(args.exp_num)
...
st = time.time()
# train = np.load('../eggdata/TrainData/train_non-processing_0210.npy',mmap_mode='r')
# val = np.load('../eggdata/TrainData/valid_non-processing_0210.npy',mmap_mode='r')
...
model = get_efficientunet_b4(out_channels=1, concat_input=True, pretrained=False,mode = 'phase', bn = BatchNorm2dSync)
model.cuda()
...
if verbose:
    best_val = np.inf
    writer = SummaryWriter('../logs/phase-only/%s/'%args.exp_num)   
...
for epoch in range(n_epoch): 
    ...
    for idx,(_x,_y) in enumerate(tqdm(train_loader,disable=(verbose==0))):
        ...
        pred_signal_recon = stftTool.inverse(y_train_mask*torch.exp(y_train_mag),pred,n_sample)
    ...
    for idx,(_x,_y) in enumerate(tqdm(valid_loader,disable=(verbose==0))):
        ...
        with torch.no_grad():
            ...
            pred_signal_recon = stftTool.inverse(y_val_mask*torch.exp(y_val_mag),pred,n_sample)
