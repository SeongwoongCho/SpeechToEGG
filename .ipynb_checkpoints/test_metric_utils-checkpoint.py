...
def detectgroundwaveletgci(EGG,peak_range=(7,15)):
    ...
    DEGG[DEGG>0]=0
    GCI = find_peaks_cwt(-DEGG,np.arange(*peak_range))
    return GCI

def corrected_naylor_metrics(ref_signal, est_signal):
#     input : goi or gci signals
...
def extract_quotient_metrics(true_egg, estimated_egg, detrend_egg=False, fs=16000):
    ...
    estimated_gci = detectgroundwaveletgci(estimated_egg)
    estimated_gci = medfilt(estimated_gci, 3)
