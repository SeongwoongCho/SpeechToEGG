...
st = time.time()
train,val = load_stft_datas_path(is_test,pseudo='6-re-5761')
train = [np.load(t,mmap_mode = 'r') for t in tqdm(train)]
val = [np.load(v,mmap_mode = 'r') for v in tqdm(val)]
...
train_loader = data.DataLoader(dataset=train_dataset,
                               ...
                               num_workers= 0 if ddp else mp.cpu_count(),
...
# t = time.time()
# for idx,(x,y,z) in enumerate(tqdm(train_loader,disable=(verbose==0))):
#     print(args.local_rank,time.time()-t)
#     t = time.time()

t = time.time()
for idx,(x,y) in enumerate(tqdm(valid_loader,disable=(verbose==0))):
    print(args.local_rank,time.time()-t)
    t = time.time()