...
class Conv2dSamePadding(nn.Conv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, image_size = [257, 64], stride=1, dilation=1, groups=1, bias=True, name=None):
        ...
        sh, sw = self.stride
        oh, ow = math.ceil(float(ih) / sh), math.ceil(float(iw) / sw)
...
class BatchNorm2dSync(nn.SyncBatchNorm):
...
class MBConvBlock(nn.Module):
    ...
    def __init__(self, block_args, global_params, idx, bn = BatchNorm2d):
        super().__init__()
        ...
        if self.block_args.expand_ratio != 1:
            ...
            self._bn0 = bn(num_features=out_channels,
        ...
        self._bn1 = bn(num_features=out_channels,
                                momentum=self.batch_norm_momentum,
                                eps=self.batch_norm_epsilon,
        ...
        self._bn2 = bn(num_features=final_output_channels,
                                momentum=self.batch_norm_momentum,
                                eps=self.batch_norm_epsilon,
